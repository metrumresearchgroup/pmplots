...
#   if(!is.null(identity)) p <- p + do.call(geom_abline,identity)
#   if(!is.null(hline)) p <- p + do.call(geom_hline,hline)
#   if(!is.null(smooth)) p <- p + do.call(geom_smooth,smooth)
...
#   }
#
#   scatt(df,x,y,xs=xs,ys=ys,group=group,...)
# }
#
...
# ##' @export
# ##' @rdname cont_cont
# eta_cont <- function(df, x, y, smooth = gs(), ...) {
#   out <- vector(mode="list", length=length(y))
#   for(i in seq_along(y)) {
#     out[[i]] <- cont_cont(df, x, y[i], smooth  = smooth, ...)
#   }
#   return(out)
# }
#
...
#   require_numeric(df, x[1])
#   require_numeric(df, y[1])
#   scatt_hs(df, x[1], y[1], xs = xs, ys = ys,...)
# }
#
# ##' @export
# ##' @rdname cont_cont
...
#   require_numeric(df, x[1])
#   require_numeric(df, y[1])
#   scatt_hs(df, x[1], y[1], xs=xs, ys=ys, smooth = smooth, hline = hline, ...)
# }
#
# ##' @export
# ##' @rdname cont_cont
...
#                      x="TIME", y="RES",
#                      hline = gh(), ...) {
#   y_time(df, yname=yname, x=x, y=y, hline = hline, ...)
# }
#
# ##' @export
# ##' @rdname res_time
...
#                       x="TAD", y="CWRES", hline = gh(), ...) {
#   y_time(df, yname=yname, xname=xname, x=x, y=y, hline = hline, ...)
# }
#
# ##' @export
# ##' @rdname res_time
...
#                      xname="value", hline = gh(), ...) {
#   require_numeric(df,x)
#   require_numeric(df,y)
#   xs$name <- paste0("Population predicted ", xname)
#   ys$name <- "Residual"
#   scatt(df, x, y, xs, ys, hline = gh(), ...)
# }
#
# ##' @export
# ##' @rdname res_pred
...
#   scatt(df, x, y, xs, ys, hline = hline, ...)
# }
#
# ##' @export
# ##' @rdname res_pred
